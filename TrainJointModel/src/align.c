...
int local_iter=0, binary = 0, debug_mode = 2, window = 5, min_count = 0, num_threads = 1, min_reduce = 1, cw = 0, sg = 1;
...
void LearnVocabFromTrainFile() {
    ...
    p_model->file_size = ftello64(fin);
    fclose(fin);
}
...
void *TrainTextModelThread(void *id) {
	//printf("Here is TrainTextModelThread 1\n");
    long long a, b, d, word=-1, last_word, sentence_length = 0, sentence_position = 0;
    long long word_count = 0, last_word_count = 0, sen[MAX_SENTENCE_LENGTH + 1];
    long long l1, l2, c, target, label;
	printf("Here is TrainTextModelThread, id:%ld\n", (long)id);
    unsigned long long next_random = (long long)id;
	//printf("Here is TrainTextModelThread2*, next_random:%lld\n", next_random);
    int anchor_pos = -1, word_begin[MAX_MENTION], words_in_mention = 1;
    char item[MAX_STRING], tmp_item[MAX_STRING];
    size_t tmp_item_len = 0;
    real f, g;
    clock_t now;
	//printf("Here is TrainTextModelThread 3\n");
	...
	//printf("Here is TrainTextModelThread 4\n");
    for(a=0;a<MAX_MENTION;a++) word_begin[a] = 0;
    while (1) {
		...
		//printf("Here is TrainTextModelThread 2, word:%lld\n", word);
        if (word == -1) continue;
        for (c = 0; c < layer1_size; c++) neu1[c] = 0;
        for (c = 0; c < layer1_size; c++) neu1e[c] = 0;
        next_random = next_random * (unsigned long long)25214903917 + 11;
        b = next_random % window;
		//printf("Here is TrainTextModelThread 2*, start training skip-gram, word:%lld, id:%ld\n", word, (long)id);
        //train skip-gram
        for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
            ...
            for (c = 0; c < layer1_size; c++) neu1e[c] = 0;
            // NEGATIVE SAMPLING
			///printf("Here is TrainTextModelThread 2, negative sampling, negative:%d, a:%lld, id:%ld\n", negative, a, (long)id);
            if (negative > 0){
				//printf("*****");
		...
		//printf("Here is TrainTextModelThread 3, training skip-gram done, id:%ld\n", (long)id);
    }
    fclose(fi);
    free(neu1);
    free(neu1e);
...
void *TrainKgModelThread(void *id) {
	//printf("Here is TrainKgModelThread 1\n");
...
void *TrainJointModelThread(void *id) {
    ...
    real f, g;
    clock_t now;
	//printf("Here is TrainJointModelThread\n");
...
void TrainTextModel(){
    ...
    for (a = 0; a < num_threads; a++) {
		if(pthread_create(&pt[a], NULL, TrainTextModelThread, (void *)a)) printf("pthread_create error, %ld\n", a);
		//else printf("creat pthread %ld, total:%d\n", a, num_threads);
		
	}
    for (a = 0; a < num_threads; a++){
		if(pthread_join(pt[a], NULL)) printf("pthread_create error, %ld\n", a);
		//else printf("join pthread %ld, total:%d\n", a, num_threads);
	}
    p_model = NULL;
}
...
int main(int argc, char **argv) {
    ...
    if ((i = ArgPos((char *)"-train_text", argc, argv)) > 0) {strcpy(text_model.train_file, argv[i + 1]); printf("text file:%s\n", text_model.train_file);}
    if ((i = ArgPos((char *)"-train_kg", argc, argv)) > 0) {strcpy(kg_model.train_file, argv[i + 1]); printf("kg file:%s\n", kg_model.train_file);}
    ...
    while(local_iter<iter){
        local_iter++;
        printf("Start training the %d time...\n ", local_iter);
        TrainTextModel();
